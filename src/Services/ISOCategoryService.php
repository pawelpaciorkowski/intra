...
final class ISOCategoryService
{
    private $selectedISOCategory;
    ...
    public function setOrder(array $data): bool
    {
        ...
        try {
            foreach ($data as $row) {
                if ($row->id) {
                    $category = $this->entityManager->getRepository(ISOCategory::class)->find($row->id);
                    ...
                    if ($row->parent_id) {
                        $category->setParent($this->entityManager->getReference(ISOCategory::class, $row->parent_id));
                    } else {
                        $category->setParent();
                    }
                }
    ...
    private function loopDetection(): void
    {
        ...
        $count = $this
            ->entityManager
            ->getRepository(ISOCategory::class)
            ->fetchAllCount();
    ...
    public function updateISOCategory(): self
    {
        $this->updateBranch();
        $this->entityManager->flush();
    ...
    private function updateBranch(?ISOCategory $parent = null, $depth = 0, $lft = 1): int
    {
        $categories = $this
            ->entityManager
            ->getRepository(ISOCategory::class)
